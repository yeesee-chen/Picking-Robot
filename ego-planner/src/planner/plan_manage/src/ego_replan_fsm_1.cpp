...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    /* callback */
    exec_timer_ = nh.createTimer(ros::Duration(0.025), &EGOReplanFSM::execFSMCallback, this);
...
void EGOReplanFSM::goal_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    if (success)
    {
        ...
        else if ((exec_state_ == EXEC_TRAJ) || (exec_state_ == ADJUST_POSE) || (exec_state_ == REPLAN_TRAJ))
        {
            changeFSMExecState(GEN_NEW_TRAJ, "TRIG");
            is_target_receive=true;
        }
