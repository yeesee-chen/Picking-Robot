...
void I2C1_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);							//打开GPIO
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);							//打开复用
	...
	GPIO_Init(GPIOB,&GPIO_InitStructure);											//GPIO设置初始化
	...
	I2C_InitStruct.I2C_ClockSpeed = 1000000;										//设置数据传输速度为HZ（0-3 000 000）
...
void I2C1_Write_Byte(u8 device_address,u8 register_address,u8 data)
{
	...
	I2C_Send7bitAddress(I2C1,device_address,I2C_Direction_Transmitter);				//发送I2C器件地址
	
	while(!I2C_CheckEvent(I2C1,I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED));		//I2C地址发送完毕
	I2C_SendData(I2C1,register_address);											//发送 命令地址
...
u8 I2C1_Read_Data(u8 device_address,u8 register_address)
{
	...
	while(!I2C_CheckEvent(I2C1,I2C_EVENT_MASTER_MODE_SELECT));						//stm32设置为主模式完毕
	I2C_Send7bitAddress(I2C1,device_address,I2C_Direction_Receiver);				//发送I2C器件地址
